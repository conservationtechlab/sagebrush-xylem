...
async def main_page():
    ...
    with ui.element('div').classes('relative w-full h-screen'):
        ...
        with ui.element('div').classes('fixed bottom-0 left-0 right-0 z-[9999] px-4 pb-4'):
            with ui.card().classes('w-full bg-slate-900/75 border border-slate-700 p-3'):
                label = ui.label(fmt(times[idx['value']])).classes('text-xs text-slate-200')
