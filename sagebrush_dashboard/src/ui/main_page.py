...
async def main_page():
    ...
    with ui.element('div').classes('relative w-full h-screen'):
        ...
        def toggle_panel():
            ...
            panel.classes(
                remove='-translate-x-full' if panel_open["value"] else 'translate-x-0',
                add='translate-x-0' if panel_open["value"] else '-translate-x-full',
            )

        ui.button(icon='layers', on_click=toggle_panel).classes(
            'fixed left-4 top-20 z-[9999] bg-white rounded-full shadow-lg'
        )

        panel = ui.element('div').classes(
            'fixed left-0 top-0 h-screen w-96 bg-white z-[9998] '
            'shadow-xl transform -translate-x-full transition-transform'
        )

        with panel:
            with ui.row().classes('items-center justify-between px-4 py-3 border-b'):
                ui.label("Layer List").classes("font-semibold")
                ui.button(icon='close', on_click=toggle_panel).props('flat')

            with ui.column().classes('p-4 gap-2'):
                for category, submap in grouped.items():
                    with ui.expansion(category, icon='category'):
